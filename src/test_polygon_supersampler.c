/*
  David J. Anderson
  Assignment 4, extension test. Produces the same image as test4a.c, but
  supersampled to superpixels of size 4x4.
*/
#include <stdio.h>
#include <stdlib.h>
#include <math.h>
...
int main(int argc, char *argv[]) {
  ...
  polygon_drawFill_SuperSampled(p, src, Blue);
  ...
  polygon_drawFill_SuperSampled(p, src, Red);
  ...
  polygon_drawFill_SuperSampled(p, src, Orange);
  ...
  polygon_drawFill_SuperSampled(p, src, White);

  printf("writing output\n");
  image_write(src, "test4a_supersampled.ppm");
