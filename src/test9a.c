...
int main(int argc, char *argv[]) {
  ...
  ds = drawstate_create();
	// Point v;
  // Vector normVPN;
  // vector_copy(&normVPN, &view.vpn);
  // vector_normalize(&normVPN);
  // point_copy(&v, &view.vrp);
  // point_set3D(&v, v.val[0] - normVPN.val[0] * view.d,
  //           v.val[1] - normVPN.val[1] * view.d,
  //           v.val[2] - normVPN.val[2] * view.d);
